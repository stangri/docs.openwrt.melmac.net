...
| Basic          | dest_ipset               | string        | dnsmasq.ipset | Enable/disable use of one of the ipset options for compatible remote policies (policies with only a remote address and no other fields set). This speeds up service start-up and operation. Currently supported options are `0`, `ipset` and `dnsmasq.ipset` (see [Use DNSMASQ ipset](#use-dnsmasq-ipset) and <sup>[7](#footnote7)</sup> for more details). Make sure the [requirements](#requirements) are met. |
| Basic          | src_ipset                | string        | 0             | Enable/disable use of `ipset` entries for compatible local policies (policies with only a local IP address or MAC address and no other fields set). Using `ipset` for local IPs/MACs is faster than using `iptables` rules, however it makes it impossible to enforce policies priority/order. Currently supported options are `0` and `ipset`. Make sure the [requirements](#requirements) are met.             |
