...
Two example custom user-files are provided: `/usr/share/pbr/pbr.user.aws` and `/usr/share/pbr/pbr.user.netflix`. They are provided to pull the AWS and Netflix IP addresses into the `pbr_wan_dst_net` ipset.
...
For `pbr` package version 0.3.4 and up you can use the following script which will only reload what's relevant to the reloaded interface:
...
-   creates ipset and resolver's ipset (like `dnsmasq.ipset`)-based policies respecting policy's priority.
